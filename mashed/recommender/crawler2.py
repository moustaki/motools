...
?episode pc:playcount ?pc.
	...
	for row in g.query(query):
		#print "episode %s played artist %s times<br>" % row
		k = k+1
		t = int(row[1])
		episodes.append((clean_ep_uri(row[0]),t))
	episodes.sort(key=operator.itemgetter(1))
	episodes.reverse()
	for (episode,pc) in episodes:
		#print "brand %s played artist %s times" % (brand,pc)
		#print brand
		g.load(fix_uri(episode))
		g.commit()
		q2 = "SELECT ?v WHERE {<%s> <http://purl.org/ontology/po/version> ?v.}" % episode
		#print q2
		for v in g.query(q2):
			#print v
			g.load(fix_uri(v[0]))
...
def clean_ep_uri(uri) :
	return uri.split('#')[0]+"#episode"
