...
class _Request(object):
	...
	def _get_signature(self):
		...
		for name in keys:
			string += name
			string += urllib.quote(self.params[name].encode('utf-8'))
...
class Artist(_BaseObject, _Taggable):
	...
	def get_similar(self, limit = None):
		...
		mbids = _extract_all(doc, 'mbid')

		artists = []
		for idx, name in enumerate(names):
			artists.append([Artist(name, *self.auth_data), mbids[idx], float(matches[idx])])
...
class _ScrobblerRequest(object):
	...
	def _check_response_for_errors(self, response):
		...
		if status_line == "OK":
			...
			raise BadAuthenticationException() #@UndefinedVariable
